...
\begin{parts}
  ...
  $\Arg z = \arctan((3/8)/(-3/8)) = \arctan(-1) = 3\pi /4$ \\
...
\begin{parts}
  ...
  \part $(\re z) (\im z) > 1$ \\
        ...
        Then we split into two cases: $x < 0$ and $x > 0$.
        If $x < 0$, we want $y < 1/x$ and if $x > 0$ we want $ y > 1/x$.
        \begin{center}
        \begin{tikzpicture}
            \draw[domain=-4:-0.25, pattern=crosshatch, pattern color=gray!60] plot (\x,1/\x) -- (-4,-4) -- (-4,-0.25);
            \draw[domain=0.25:4, pattern=crosshatch, pattern color=gray!60] plot (\x,1/\x) -- (4,4) -- (0.25,4);
            \draw[color=white, domain=-4:-0.25] plot (\x,1/\x) -- (-4,-4) -- (-4,-0.25);
